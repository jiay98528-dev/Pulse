...
> 当前版本: v2.0 | 日期: 2026-06-21 | 最高设计权威: docs/需求规格说明书_v2.0.md | M1-M6 ✅ | 发行就绪
...
| Tab | 定位 | 说明 |
|-----|------|------|
| **仪表盘** | 远距扫视首页 | 可配置KPI组件墙 (3尺寸+拖拽)，自适应AI区块，费用限额告警 |
| **硬件** | 靠近细看 | 系统详情 + LAN设备横栏 + 扫描局域网弹窗 |
| **分析** | CSV数据分析 | 拖入zip→智能解析→表格+4图表+对比模式，数据来源标记 |
| **配置** | 设置与外观 | AI供应商/系统/外观/主题市场/关于，主题市场独立弹窗 |
| **插件** | 扩展管理 | 极简卡片列表 + LAN监控/Token采集等，配对授权弹窗 |

## v2.0 新增功能 (Post-M6)
- 主题市场独立弹窗 (720px 网格浏览，离线降级提示)
- LAN 扫描可视化窗口 (5秒倒计时，设备列表，配对入口)
- 费用限额告警 (日/月超限红色横幅)
- Deepseek ZIP 双CSV导入 (utc_date透视 + 去重覆盖)
- 数据来源标记 (分析页显示导入文件名+时间)
